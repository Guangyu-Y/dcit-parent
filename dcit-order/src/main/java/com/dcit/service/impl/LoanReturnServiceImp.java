...
public class LoanReturnServiceImp implements LoanReturnService{
	...
	public Boolean RecordLoanReturnInfo(LoanReturn loanReturn,Double interesrRate,String starttime) {
		...
		System.out.println(loanReturn.getSurplusnumber());
			...
			loanReturn.setInterest(interesrRate*LoanDayCount*loanReturn.getSumnumber()/360);   
			//写入数据库
	...
	public List<LoanReturn> listRecordLoanInfo(String loancode) {
		...
		LoanReturn temp = new LoanReturn();
		List<LoanReturn> LoanReturnList = loanReturnMapper.select(loanReturn);
		for(int i=0;i<LoanReturnList.size();i++){
			System.out.println(LoanReturnList.get(i).getWebsitecode());
			String websitecode = LoanReturnList.get(i).getWebsitecode();
			if(websitecode!=null)
			{
				website = websiteService.findWebsiteById(websitecode);
				if(website!=null)
				{
					 LoanReturnList.get(i).setWebsitecode(website.getName());
				}
			}
